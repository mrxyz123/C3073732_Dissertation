...
class TestMetricFunctions(unittest.TestCase):
    ...
    def test_update_statistics(self, *mocks):
        data = pd.DataFrame({'Value': [1, 2, 3, 4, 5]})
        update_statistics(data)
        self.assertEqual(mocks[0].text, " 3.000")  # mean
        self.assertEqual(mocks[1].text, " 3.000")  # median
...
class TestIntegration(unittest.TestCase):
    ...
    @patch('app.read_metric_data')
    @patch('app.update_graph')
    @patch('app.mean_value_label', new_callable=MockLabel)
    @patch('app.median_value_label', new_callable=MockLabel)
    @patch('app.mode_value_label', new_callable=MockLabel)
    @patch('app.min_value_label', new_callable=MockLabel)
    ...
    @patch('app.q2_value_label', new_callable=MockLabel)
    @patch('app.q3_value_label', new_callable=MockLabel)
    def test_graph_update_integration(self, *mocks):
        mock_update_graph = mocks[1]
        mock_read_metric_data = mocks[0]
        mock_read_metric_data.return_value = pd.DataFrame({
            'Country Name': ['USA'],
            '2000': [100],
            '2001': [200]
        })
        
        metric_var = MagicMock()
        country_var = MagicMock()
